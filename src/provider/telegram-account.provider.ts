...
class TelegramAccountProvider extends ProviderClass<TelegramEvents> {
  ...
  constructor(args?: TelegramAccountArgs) {
    ...
    if (!this.globalVendorArgs.apiId) {
      throw new Error(
        "Must provide Telegram API ID. Visit: https://my.telegram.org/auth?to=apps"
      );
    }
    if (!this.globalVendorArgs.apiHash) {
      throw new Error(
        "Must provide Telegram API Hash. Visit: https://my.telegram.org/auth?to=apps"
      );
    }
  ...
  async getRespondedConversations(args?: IterDialogsParams) {
    ...
    for await (const dialog of this.client.iterDialogs(args)) {
      if (dialog.unreadCount == 0) {
        ...
        if (message.senderId.toString() == mySenderId)
          conversationsIds.push(message);
      }
    }
  ...
  async saveFile(
    ctx: { message: Api.Message; from: EntityLike },
    options: { path: string }
  ): Promise<string> {
    try {
